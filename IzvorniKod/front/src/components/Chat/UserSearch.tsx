...
const UserSearch: React.FC<UserSearchProps> = ({ onUserSelect, setIsSearching }) => {
	...
	const handleSearch = () => {
		...
		fetch(`${import.meta.env.VITE_BACKEND_API}/users/search?query=${encodeURIComponent(searchTerm)}`, {
			method: "GET",
			headers: {
	...
	return (
		<div className="p-2">
			...
			<div className="flex justify-center">
				<p className="text-gray-600 text-lg font-semibold">Find a new person you want to chat with!!!</p>
			</div>
			...
			<ul className="max-h-72 overflow-y-auto mt-3 scrollbar scrollbar-thumb-[#5434ab] scrollbar-track-transparent">
				{users.map((user) => (
					<li
						...
						className="p-2 cursor-pointer text-black flex items-center gap-3"
					>
						<img
							src={
								user.profilePicture ||
